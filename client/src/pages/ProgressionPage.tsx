  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">📈 Progressive Overload</h1>
      ...
      {(overview?.ready.length ?? 0) > 0 && (
        <div className="space-y-3">
          <h2 className="font-semibold text-brand-700 flex items-center gap-2">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSelectedExercise(null)} className="text-xs text-gray-400 hover:text-gray-600">
                ✕ Close
              </button>
            </div>
          </CardHeader>
          <CardBody>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={chartData}>
                ...
                />
                <Line yAxisId="weight" type="monotone" dataKey="weight" stroke="#16a34a" strokeWidth={2} dot={{ r: 4 }} name="weight" />
                <Line yAxisId="reps" type="monotone" dataKey="reps" stroke="#60a5fa" strokeWidth={2} dot={{ r: 4 }} strokeDasharray="4 2" name="reps" />
              </LineChart>
            </ResponsiveContainer>
            <div className="flex gap-4 justify-center mt-2 text-xs text-gray-500">
              <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-brand-600 inline-block" /> Weight ({units.weightUnit})</span>
              <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-blue-400 inline-block" /> Avg reps / set</span>
            </div>
          </CardBody>
        </Card>
      )}
      ...
      <CardBody className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-semibold text-gray-900">{s.exerciseName}</p>
          ...
          <p className="text-xs text-gray-500 mt-0.5">{reasonDisplay}</p>
        </div>
        ...
        <div className="flex items-center gap-6 shrink-0">
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-gray-700">
              ...
              <span className="text-xs font-normal text-gray-400 ml-0.5">{units.weightUnit}</span>
            </p>
            <p className="text-xs text-gray-400">{s.currentReps} reps avg</p>
          </div>
          {s.readyForProgression && (
            <>
              ...
              <div className="text-center">
                ...
                {s.suggestedReps && (
                  <p className="text-xs text-gray-400">aim {s.suggestedReps}+ reps</p>
                )}
              </div>
            </>
          )}
